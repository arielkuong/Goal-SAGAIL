...
from learn_utils.models import actor, critic
...
class LfD_ddpg_agent:
    ...
    def learn(self):
        ...
        # store the expert episodes
        self.expert_buffer.store_episode([mb_obs, mb_ag, mb_g, mb_actions])
        self.buffer.store_episode([mb_obs, mb_ag, mb_g, mb_actions], is_demo = True)
    ...
    def _update_network_ddpg(self):
        ...
        inputs_next_tensor = inputs_next_tensor.to(self.device)
        ...
        with torch.no_grad():
            ...
            q_next_value = self.critic_target_network(inputs_next_tensor, actions_next)
        ...
        # the q loss
        real_q_value = self.critic_network(inputs_tensor, actions_tensor)
        critic_loss = (target_q_value - real_q_value).pow(2).mean()
        # the actor loss
        actions_real = self.actor_network(inputs_tensor)
        actor_loss = -self.critic_network(inputs_tensor, actions_real).mean()
    ...
    #     transitions = self.buffer.sample(self.args.batch_size)
    #
    #     demo_transitions = self.expert_buffer.match_sample_nearest_neighbour(transitions['obs'], transitions['g'], self.env.compute_reward)
    #     o, g = self._preproc_og(demo_transitions['obs'], demo_transitions['g'])
    #     o_next, g_next = self._preproc_og(demo_transitions['obs_next'], demo_transitions['g'])
    ...
    #     actions_tensor = torch.tensor(demo_transitions['actions'], dtype=torch.float32)
    #     r_tensor = torch.tensor(demo_transitions['r'], dtype=torch.float32)
    #     if self.args.cuda:
    #         inputs_tensor = inputs_tensor.cuda()
    #         inputs_next_tensor = inputs_next_tensor.cuda()
