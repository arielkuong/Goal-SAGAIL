...
"""
GOAL-SAGAIL for goal-based task with HER

"""
class Goal_sagail_agent:
    def __init__(self, args, env, env_params):
        ...
        self.none_expert_buffer = replay_buffer(self.env_params, self.args.buffer_size, self.her_module.sample_her_transitions)
        self.expert_buffer = replay_buffer(self.env_params, self.args.expert_buffer_size*env_params['max_timesteps'], self.her_module.sample_her_transitions)
        ...
        print('Match distance limit = {}'.format(self.args.match_dis_limit))
        ...
        self.save_name_suffix = 'goalsgail_new_suboptimal_gailweight' + str(self.args.gail_weight) + gail_anneal_name +\
                                '_matchdislimit' + str(self.args.match_dis_limit) + '_rewardD_demo' + str(self.args.demo_length) + demo_anneal_name
    ...
    def learn(self):
        ...
        expert_added_size += mb_obs.shape[0]
        ...
        for epoch in range(self.args.n_epochs):
            for cycle in range(self.args.n_cycles):
                ...
                print('[{}] Collecting data for epoch{}, cycle {}'.format(datetime.now(), epoch, cycle))
                mb_obs, mb_ag, mb_g, mb_actions = [], [], [], []

                expert_obs, expert_ag, expert_g, expert_actions = [], [], [], []
                none_expert_obs, none_expert_ag, none_expert_g, none_expert_actions = [], [], [], []
                expert_transfer_counter = 0
                none_expert_transfer_counter = 0

                for rollout in range(self.args.num_rollouts_per_cycle):
                    # reset the rollouts
                    ...
                    initial_g = observation['achieved_goal']
                    desired_g = observation['desired_goal']
                    ep_r = 0
                    ...
                    for t in range(self.env_params['max_timesteps']):
                        ...
                        observation_new, r, _, _, info = self.env.step(action)
                        #self.env.render()
                    ...
                    # start match sample and compare for goal-sagail
                    if(info['is_success']):
                        # Get the most similar expert episode in the expert buffer. also calculate the combined distance
                        episode_reward, combined_dis, expert_idx = self.expert_buffer.match_episode(initial_g, desired_g, self.args.env_name, self.env.unwrapped.compute_reward)

                        if combined_dis > self.args.match_dis_limit and ep_r < -2:
                            # combined distance larger than threshold, regard as a new expert episode
                            expert_obs.append(ep_obs)
                            expert_ag.append(ep_ag)
                            expert_g.append(ep_g)
                            expert_actions.append(ep_actions)
                            expert_transfer_counter += 1
                            print('[{}] Epoch {}, Cycle {}: Success rollout {} cannot find demo match, combined distance: {:.3f}, episode reward: {}'.format(datetime.now(), epoch, cycle, rollout, combined_dis, ep_r))
                        elif combined_dis <= self.args.match_dis_limit and ep_r > episode_reward and ep_r < -2:
                             # combined distance smaller than threshold and episode reward is higher, regard as a better expert episode
                            expert_obs.append(ep_obs)
                            expert_ag.append(ep_ag)
                            expert_g.append(ep_g)
                            expert_actions.append(ep_actions)
                            expert_transfer_counter += 1
                            print('[{}] Epoch {}, Cycle {}: Success rollout {} better than expert {}, combined distance: {:.3f}, episode reward: {}'.format(datetime.now(), epoch, cycle, rollout, expert_idx, combined_dis, ep_r))
                        else:
                            # combined distance smaller than threshold but episode reward is lower than expert, regard as non-expert episode
                            none_expert_obs.append(ep_obs)
                            none_expert_ag.append(ep_ag)
                            none_expert_g.append(ep_g)
                            none_expert_actions.append(ep_actions)
                            none_expert_transfer_counter += 1
                    else:
                        # Non success episode will be consider as non-expert episode
                        none_expert_obs.append(ep_obs)
                        none_expert_ag.append(ep_ag)
                        none_expert_g.append(ep_g)
                        none_expert_actions.append(ep_actions)
                ...
                # store all episodes into the overall buffer
                self.buffer.store_episode([mb_obs, mb_ag, mb_g, mb_actions])
                self._update_normalizer([mb_obs, mb_ag, mb_g, mb_actions])

                # store the new expert and non-expert episodes into corresponding buffers
                if expert_transfer_counter > 0:
                    expert_obs = np.array(expert_obs)
                    expert_ag = np.array(expert_ag)
                    expert_g = np.array(expert_g)
                    expert_actions = np.array(expert_actions)
                    self.expert_buffer.store_episode([expert_obs, expert_ag, expert_g, expert_actions])
                    expert_added_size += expert_obs.shape[0]
                    expert_buffer_length = self.expert_buffer.get_length()
                    print('[{}] Epoch {}, Cycle {}: {} trajs stored in expert buffer, Expert total size: {}'.format(datetime.now(), epoch, cycle, expert_transfer_counter, expert_buffer_length))

                if none_expert_transfer_counter > 0:
                    none_expert_obs = np.array(none_expert_obs)
                    none_expert_ag = np.array(none_expert_ag)
                    none_expert_g = np.array(none_expert_g)
                    none_expert_actions = np.array(none_expert_actions)
                    self.none_expert_buffer.store_episode([none_expert_obs, none_expert_ag, none_expert_g, none_expert_actions])
                ...
                # Update the networks
                print('[{}] Update networks for epoch {}, cycle {}'.format(datetime.now(), epoch, cycle))
                ...
                # expert_added_size_all.append(expert_added_size)
                # np.save(self.model_path + '/Goalsagail_expert_total_added_size_allcycles.npy', expert_added_size_all)
    ...
    def _update_discriminator(self):
        transitions = self.none_expert_buffer.sample(self.args.dis_batch_size)
    ...
    def _update_network_ddpg_gail(self):
        ...
        # real_transitions = self.buffer.sample(real_size)
